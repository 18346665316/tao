...
import time
import re
from timeout_test import timeoutfunc
...
class Taobao_spider():
    ...
    def __init__(self, start_page, end_page, keywords):
        ...
        self.f = open(r'data/%s-%s页.txt' % (self.start_page, self.end_page), 'a', encoding='utf-8')
    ...
    def parse_page(self, text):
        ...
        dict_new = json.loads(json_text, encoding='utf-8')
        # 调用函数获取到需要的data
        parse_data = self.get_discount_price(dict_new)
        if parse_data:
            print(json.dumps(parse_data), file=self.f, flush=True)
    def get_discount_price(self, dict_new):
        ...
        try:
            if self.driver.find_element_by_id('J_PromoPrice').find_element_by_tag_name('dd').text == '':
            ...
            print('出现未知错误')
            return None
        dict_new["detail"]["defaultdiscountprice"] = price_all
        # 获取所有图片标签,包括隐藏
